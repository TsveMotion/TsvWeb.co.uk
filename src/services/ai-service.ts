...
export const AIService = {
  // Generate blog content using GPT-4o with real-time data
  generateBlogContent: async (request: BlogGenerationRequest): Promise<BlogGenerationResponse> => {
    try {
      ...
      const targetAudience = request.targetAudience || 'business owners';
      ...
      if (request.keyPoints && request.keyPoints.length > 0) {
        for (let i = 0; i < Math.min(request.keyPoints.length, sectionCount); i++) {
          sectionHeadings.push(request.keyPoints[i]);
        }
        
        // If we need more sections than provided key points
        for (let i = request.keyPoints.length; i < sectionCount; i++) {
